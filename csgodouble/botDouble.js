...
//Latest screenshot: https://raw.githubusercontent.com/ajkreamer/javascript/master/bots/botDoubleProgress.png
			...
			'<div class="col-lg-4">' +
				...
				'<div class="form-group">' +
					'<div class="input-group">' +
						...
						'<input type="number" class="form-control" placeholder="Calculating wager..." id="wagerValue" disabled>' +
					'</div>' +
				'</div>' +
				'<div class="form-group">' +
					'<div class="input-group">' +
						'<div class="input-group-addon" id="betOpts"><input id="betGreen" type="checkbox"> &nbsp; Green bet</div>' +
						'<input type="number" class="form-control" placeholder="Calculating green wager..." id="wagerGreen" disabled>' +
					'</div>' +
				'</div>' +
				'<div class="btn-group">' +
...
function betSet(i) {
	switch (i) {
		case 0: document.getElementById('botColor').innerHTML = '<font color="#c9302c">Red</font>';
			break;
		case 1: document.getElementById('botColor').innerHTML = '<font color="#000000">Black</font>';
			break;
		case 2: val = Math.floor((Math.random() * 2) + 1);
			...
			else document.getElementById('botColor').innerHTML = '<font color="#000000">R</font><font color="#c9302c">a</font><font color="#000000">i</font><font color="#c9302c">n</font><font color="#000000">b</font><font color="#c9302c">o</font><font color="#000000">w</font>';
			break;
		...
		case 4: string = "";
			...
			else string = string + '<font color="#000000">R</font>';
			val = Math.floor((Math.random() * 2) + 1);
			if (val == 1) string = string + '<font color="#c9302c">a</font>';
			else string = string + '<font color="#000000">a</font>';
			val = Math.floor((Math.random() * 2) + 1);
			if (val == 1) string = string + '<font color="#c9302c">n</font>';
			else string = string + '<font color="#000000">n</font>';
			val = Math.floor((Math.random() * 2) + 1);
			if (val == 1) string = string + '<font color="#c9302c">d</font>';
			else string = string + '<font color="#000000">d</font>';
			val = Math.floor((Math.random() * 2) + 1);
			if (val == 1) string = string + '<font color="#c9302c">o</font>';
			else string = string + '<font color="#000000">o</font>';
			val = Math.floor((Math.random() * 2) + 1);
			if (val == 1) string = string + '<font color="#c9302c">m</font>';
			else string = string + '<font color="#000000">m</font>';
		...
		if (rollidNew > rollid) {
			...
			switch (strat) {
				case 0: betWager = betBase;
					break;
				case 1: if (betBase < 3) betBase = 3;
					if (result == 0) {
						betWager = betBase;
						wager[0] = betBase;
						wager[1] = betBase;
					}
					else if (result == 1) betWager = betBase;
					else if (result == 2) betWager = wager[1] * 2;
					else if (result == 3) betWager = betBase;
					else betWager = betBase;
					break;
				case 5: if (betBase < 3) betBase = 3;
					if (result == 0) {
						betWager = betBase;
						wager[0] = betBase;
						wager[1] = betBase;
					...
					else if (result == 2) betWager = wager[1] * 3;
					else if (result == 3) betWager = betBase;
					else betWager = betBase;
					break;
			}
